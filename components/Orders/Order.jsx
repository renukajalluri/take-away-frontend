...
useEffect(()=>{
    ...
    const acceptOrder = async(order,token) =>{
        try{
            const res = await axios.get(`http://localhost:4000/order/accept/processing/${order}`,{
                    headers: {
            ...
            <Box sx={style}>
                ...
                {orders.map(ord=>{
                    return(
                      ...
                      <span className={classes.itemName}>{ord.item.name}</span>
                      <span className={classes.itemDesc}>{ord.item.description}</span>
                      <span className={classes.amount}>£{ord.item.cost} x {ord.quantity}</span>
                      </div>
                      </div>
    ...
    return (
        <div className={classes.order}>
            ...
            {customerOrders?customerOrders.map(order=>{
                ...
                return (
                    <>
                    <OrderCard order={order} orders={orders} total={total} items={items} getOrders ={getOrders}/>
                    </>
